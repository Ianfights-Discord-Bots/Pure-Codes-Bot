...
function openTicket(interaction) {
    try {
        client.guilds.cache.get(guildId).channels.create({
            name: `account-ticket-${interaction.user.username}`,
            ...
            //@ts-ignore
            client.channels.cache.get(m.id).send({ content: `Welcome <@${interaction.user.id}> , thank you for choosing Pure Codes! One of the <@&997585123430113310> will be with you soon. Please check <#1038224405920235542> to see our custom stock, or make a request here!`, components: [row] });
